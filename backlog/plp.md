...
| F3 | Quick-filter chip row (top facet values shown inline, no drawer) `[C]` | `Facets` container → `Facet` / `FacetBucketLabel` / `SelectedFacets` slots | 🟡 | Build a custom slot renderer that surfaces the top N facet buckets as toggleable pills above the grid, kept in sync with the popover `Facets` state (same underlying search context). | — | Medium | Low |
...
None remaining for this scope — all decisions are captured in Assumptions above.
